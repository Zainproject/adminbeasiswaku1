...
@section('main1')
    <main class="ease-soft-in-out xl:ml-68.5 relative h-full max-h-screen rounded-xl transition-all duration-200">
        ...
        <form method="POST" action="{{ url('beasiswa/' . $beasiswa->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <h6 class="mb-4">Edit Beasiswa</h6>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                {{-- Penyedia --}}
                <div>
                    <label class="block mb-1 text-xs font-semibold text-slate-600">Penyedia</label>
                    <input type="text" name="penyedia" value="{{ old('penyedia', $beasiswa->penyedia) }}"
                ...
                <div>
                    ...
                    <input type="text" name="nama_beasiswa" value="{{ old('nama_beasiswa', $beasiswa->nama_beasiswa) }}"
                ...
                <div>
                    ...
                    <input type="date" name="deadline"
                        value="{{ old('deadline', $beasiswa->deadline ? $beasiswa->deadline->format('Y-m-d') : '') }}"
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        <option value="active" {{ old('status', $beasiswa->status) == 'active' ? 'selected' : '' }}>Active
                        </option>
                        <option value="inactive" {{ old('status', $beasiswa->status) == 'inactive' ? 'selected' : '' }}>
                            Inactive</option>
                        <option value="closed" {{ old('status', $beasiswa->status) == 'closed' ? 'selected' : '' }}>Closed
                        </option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi" rows="4"
                        class="w-full px-3 py-2 text-sm border rounded-lg @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $beasiswa->deskripsi) }}</textarea>
                </div>
            </div>
